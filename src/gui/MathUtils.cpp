...
tk::common::Vector2<float> focalFromFOV(const tk::common::Vector2<float> fov, tk::common::Vector2<unsigned int> imageSize)
{
    return tk::common::Vector2<float>{imageSize.x / (2.0f * (float)tan(fov.x * 0.5f)),
                imageSize.y / (2.0f * (float)tan(fov.y * 0.5f))};
}

// see https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
void quaternionToEulerianAngle(float q[4], float& roll, float& pitch, float& yaw)
{
    float ysqr = q[1] * q[1];
    ...
    float t4 = +1.0 - 2.0 * (ysqr + q[2] * q[2]);
    yaw = atan2(t3, t4);
}

tk::common::Vector3<float> pos2DTo3D(const tk::common::Vector2<float> &in)
{
    tk::common::Vector3<float>  out;
    out.x = in.x;
    out.y = in.y;
...
tk::common::Vector2<float> pos3DTo2D(const tk::common::Vector3<float> &in)
{
    tk::common::Vector2<float>  out;
    out.x = in.x;
    out.y = in.y;
