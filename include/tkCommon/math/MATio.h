...
public:
  ...
  template <matio_compression compression = MAT_COMPRESSION_NONE>
  int write_comment(const char * matname, const char* comment) {

      size_t    dims[2];
      dims[0] = 1;
      dims[1] = strlen(comment);

      matvar_t * var = Mat_VarCreate(matname,MAT_C_CHAR,MAT_T_UINT8,2,
                             dims,(void*)comment,MAT_F_DONT_COPY_DATA);
